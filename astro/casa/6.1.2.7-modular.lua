-- -*- lua -*-
help([[
This module configures CASA 6.1.2.7 (modular). After loading the module on a compute node (e.g. 'srun --x11 --pty bash' then 'module load casa/6.1.2.7-modular'), use 'casapy' to launch an interactive ipython session inside the container, and then 'import casatasks, casatools'. Use 'casapy myscript.py' to run your CASA script inside an sbatch file, and 'casashell' to launch an interactive CASA session like in CASA 5.
]])
whatis("Version: 6.1.2.7")
whatis("Keywords: CASA, 6.1.2.7, 6, python3, py3")
whatis("Description: CASA 6.1.2.7 modular container")
...
local bashStr = 'singularity exec /idia/software/containers/casa-6.1.2.7-modular.simg xvfb-run -a ipython "$@"'
local cshStr = ''
