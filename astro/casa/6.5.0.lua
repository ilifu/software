-- -*- lua -*-
help([[
This module configures CASA 6.5.0 (modular). After loading the module on a compute node (e.g. 'srun --x11 --pty bash' then 'module load casa/6.5.0'), use 'casapy' to launch an interactive ipython session inside the container, and then 'import casatasks, casatools'. Use '>
]])
whatis("Version: 6.5.0.15")
whatis("Keywords: CASA, 6.5.0, 6, python3, py3")
whatis("Description: CASA 6.5.0 modular container")

local singularity_image = "/idia/software/containers/casa-6.5.0-modular.sif"
...
local bashStr = 'singularity exec /idia/software/containers/casa-6.5.0-modular.sif xvfb-run -a ipython "$@"'
local cshStr = ''
