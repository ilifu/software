-- -*- lua -*-
help([[
This module configures CASA 6.2 (modular). After loading the module on a compute node (e.g. 'srun --x11 --pty bash' then 'module load casa/6.2'), use 'casapy' to launch an interactive ipython session inside the container, and then 'import casatasks, casatools'. Use 'casapy myscript.py' to run your CASA script inside an sbatch file, and 'casashell' to launch an interactive CASA session like in CASA 5.
]])
whatis("Version: 6.2.0.124")
whatis("Keywords: CASA, 6.2, 6, python3, py3")
whatis("Description: CASA 6.2 modular container")

local singularity_image = "/idia/software/containers/casa-6.2.0.simg"
...
local bashStr = 'singularity exec /idia/software/containers/casa-6.2.0.simg xvfb-run -a ipython "$@"'
local cshStr = ''
