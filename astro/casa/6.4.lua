-- -*- lua -*-
help([[
This module configures CASA 6.4 (modular). After loading the module on a compute node (e.g. 'srun --x11 --pty bash' then 'module load casa/6.4'), use 'casapy' to launch an interactive ipython session inside the container, and then 'import casatasks, casatools'. Use 'casapy myscript.py' to run your CASA script inside an sbatch file, and 'casashell' to launch an interactive CASA session like in CASA 5.
]])
whatis("Version: 6.4.0.16")
whatis("Keywords: CASA, 6.4, 6, python3, py3")
whatis("Description: CASA 6.4 modular container")

local singularity_image = "/idia/software/containers/casa-6.4.simg"
...
local bashStr = 'singularity exec /idia/software/containers/casa-6.4.simg xvfb-run -a ipython "$@"'
local cshStr = ''
